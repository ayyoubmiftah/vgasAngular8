...
export class AddPartenaireComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.serviceC.getDeals().subscribe(response => {
      this.deal = response.data;
    });
    if (this.id !== undefined) {
      this.serviceC.getSid(this.id).subscribe(response => {
        this.sourcer = response.data;
        this.croppedImage= this.sourcer.sourcerAvatar;
        this.formPartenaire.controls.nom.setValue(this.sourcer.sourcerFirstName);
        this.formPartenaire.controls.prenom.setValue(this.sourcer.sourcerLastName);
        this.formPartenaire.controls.tel.setValue(this.sourcer.sourcerMobileNo);
        this.formPartenaire.controls.email.setValue(this.sourcer.sourcerEmail);
        this.formPartenaire.controls.id.setValue(this.sourcer.dealsCommercialById.id);
      });
    }
  }
  ...
  addPartenaire() {
    const Partenaire = {
      id: this.id,
      sourcerFirstName: this.formPartenaire.controls.nom.value,
      sourcerLastName: this.formPartenaire.controls.prenom.value,
      sourcerMobileNo: this.formPartenaire.controls.tel.value,
      sourcerAvatar: this.croppedImage,
      sourcerEmail: this.formPartenaire.controls.email.value,
      dealsCommercialById: {id: this.formPartenaire.controls.id.value}
    };
    if (this.id == null) {
      this.serviceC.addPartenaire(Partenaire).subscribe(response => {
        this.toastrService.success('Partenaire ajouté', 'Partenaire', {
          timeOut: 2000,
        });
      ...
      this.serviceC.editPartenaire(this.id, Partenaire).subscribe(response => {
        this.toastrService.success('Partenaire Modifié', 'Partenaire', {
          timeOut: 2000,
        });
        console.log(response.data);
        this.router.navigate(['/listPartenaire']);
