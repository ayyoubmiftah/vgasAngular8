import {Component, OnInit, TemplateRef} from '@angular/core';
import {ProfileService} from '../../../services/profile.service';
import {CvtModel} from '../../../models/cvt.model';
import {FormBuilder, Validators} from '@angular/forms';
import {ToastrService} from 'ngx-toastr';
import {ActivatedRoute, Router} from '@angular/router';
import {ImageCroppedEvent} from 'ngx-image-cropper';
import {Location} from '@angular/common';
import {BsModalRef, BsModalService} from 'ngx-bootstrap';

@Component({
    selector: 'app-list-profil',
    templateUrl: './list-profil.component.html',
    styleUrls: ['./list-profil.component.scss']
})
export class ListProfilComponent implements OnInit {
    cvt: CvtModel;
    id: number;
    formCvt;
    imageChangedEvent: any = '';
    croppedImage: string = 'data:image/png;base64,';
    config = {backdrop: true, ignoreBackdropClick: true, class: 'modal-lg'};
    modelRef: BsModalRef;

    constructor(private serviceC: ProfileService,
                private toastrService: ToastrService,
                private route: ActivatedRoute,
                private router: Router,
                private location: Location,
                private modelService: BsModalService,
                private formBuilder: FormBuilder) {
        this.formCvt = this.formBuilder.group({
            nom: ['', Validators.required],
            prenom: ['', Validators.required],
            dateN: ['', Validators.required],
            adress: ['', Validators.required],
            email: ['', Validators.required],
            tel: ['', Validators.required],
            gender: ['', Validators.required],
            nationality: ['', Validators.required],
            family: ['', Validators.required],
            qualification: ['', Validators.required],
            comment: ['', Validators.required],
            about: ['', Validators.required],
            availability: ['', Validators.required],
            title: ['', Validators.required],
            price: ['', Validators.required],
        })
    }

    ngOnInit() {
        this.id = this.route.snapshot.params['id'];
        if (this.id !== undefined) {
            this.serviceC.getCvid(this.id).subscribe(response => {
                this.cvt = response.data;
                this.formCvt.controls.nom.setValue(this.cvt.candidateFirstName);
                this.formCvt.controls.prenom.setValue(this.cvt.candidateLastName);
                this.formCvt.controls.dateN.setValue(this.cvt.birthDate);
                this.formCvt.controls.adress.setValue(this.cvt.candidateAddress);
                this.formCvt.controls.email.setValue(this.cvt.candidateEmail);
                this.formCvt.controls.tel.setValue(this.cvt.candidateMobileNo);
                this.formCvt.controls.gender.setValue(this.cvt.gender);
                this.formCvt.controls.nationality.setValue(this.cvt.nationality);
                this.formCvt.controls.family.setValue(this.cvt.familySituation);
                this.formCvt.controls.qualification.setValue(this.cvt.qualificationLevel);
                this.croppedImage = 'data:image/png;base64,' + this.cvt.candidateAvatar;
                this.formCvt.controls.comment.setValue(this.cvt.qualificationComment);
                this.formCvt.controls.about.setValue(this.cvt.candidateAbout);
                this.formCvt.controls.availability.setValue(this.cvt.availability);
                this.formCvt.controls.title.setValue(this.cvt.candidateProfileTitle);
                this.formCvt.controls.price.setValue(this.cvt.price);
            });
        } else {
            localStorage.setItem('suivant active', '1');
        }
    }

    suivantActiveFalse() {
        localStorage.setItem('suivant active', '0');
    }
    showPop(template: TemplateRef<any>) {
        this.modelRef = this.modelService.show(template, this.config);
    }

    changePageDetail(menu: number) {
        this.router.navigate([`/addCandidatDetail/${this.id}`]);
        localStorage.setItem('menu', `${menu}`);
    }

    imageCropped(event: ImageCroppedEvent) {
        this.croppedImage = event.base64.toString();
    }

    fileChangeEvent(event: any): void {
        this.imageChangedEvent = event;
    }

    localstorageSuivant(): boolean {
        return localStorage.getItem('suivant active') === '1';
    }

    addCvt() {
        if (this.formCvt.valid) {
            const cvt = {
                id: this.id,
                candidateFirstName: this.formCvt.controls.nom.value,
                candidateLastName: this.formCvt.controls.prenom.value,
                birthDate: this.formCvt.controls.dateN.value,
                candidateAddress: this.formCvt.controls.adress.value,
                candidateEmail: this.formCvt.controls.email.value,
                candidateMobileNo: this.formCvt.controls.tel.value,
                gender: this.formCvt.controls.gender.value,
                nationality: this.formCvt.controls.nationality.value,
                familySituation: this.formCvt.controls.family.value,
                qualificationLevel: this.formCvt.controls.qualification.value,
                candidateAvatar: this.croppedImage.split(',')[1],
                qualificationComment: this.formCvt.controls.comment.value,
                candidateAbout: this.formCvt.controls.about.value,
                availability: this.formCvt.controls.availability.value,
                candidateProfileTitle: this.formCvt.controls.title.value,
                price: this.formCvt.controls.price.value
            };
            if (this.id == null) {
                this.serviceC.addCvt(cvt).subscribe(response => {
                    this.toastrService.success('candidat ajouté', 'Candidat', {
                        timeOut: 2000,
                    });
                    this.cvt = response.data;
                    this.router.navigate([`/addCandidatDetail/${this.cvt.id}`]);
                    localStorage.setItem('menu', `1`);
                }, err => {
                    console.log(err.toString())
                });
            } else {
                this.serviceC.editCvt(this.id, cvt).subscribe(response => {
                    this.toastrService.success('candidat Modifié', 'candidat', {
                        timeOut: 2000,
                    });
                    this.router.navigate([`/listCVThéque`]);
                }, err => {
                    console.log(err.toString());
                });
            }
        } else {
            this.toastrService.error('remplir les champs Obligatoire', 'Merçi de', {
                timeOut: 3000,
            });
        }
    }
    cancelBtn(){
        this.router.navigate([`/listCVThéque`]);
    }

}
